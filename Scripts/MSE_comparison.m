...
for ID_opt = Ind{i_treat}
    ...
    % end
    VL_all = ((log10(y(2:501,3))-11.35)/(-0.25));
    VL_topeak = ((log10(y(2:ind_max_y(ID_opt),3))-11.35)/(-0.25));
    VL_afterpeak = ((log10(y(ind_max_y(ID_opt):501,3))-11.35)/(-0.25));
    areaVL(ID_opt) = sum((35-VL_all(VL_all<35))*0.1);
    areaVLtopeak(ID_opt) = sum((35-VL_topeak(VL_topeak<35))*0.1);
    areaVLafterpeak(ID_opt) = sum((35-VL_afterpeak(VL_afterpeak<35))*0.1);
    meanVL(ID_opt) = 35-areaVL(ID_opt)*10/sum(VL_all<35);
    meanVLtopeak(ID_opt) = 35-areaVLtopeak(ID_opt)*10/sum(VL_topeak<35);
    meanVLafterpeak(ID_opt) = 35-areaVLafterpeak(ID_opt)*10/sum(VL_afterpeak<35);
    ...
    CN_sympt(ID_opt) = ((log10(y(find(tspan==t_shift(ID_opt)),3))-11.35)/(-0.25));
    if ismember(i_treat,[1,6])
        CN_hosp(ID_opt) = ((log10(y(find(tspan==t_shift(ID_opt)+T_hosp_check(ID_opt)),3))-11.35)/(-0.25));
    end

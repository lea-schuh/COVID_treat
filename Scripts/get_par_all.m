...
Ind = [{inds_treat_hosp},0,{inds_wo_treat},0,{inds_hosp},[1:37,39:length(ID_check)],[1:37,39:length(ID_check)],[1:37,39:length(ID_check)],{inds_treat_hosp}];
...
%i_treat = 9; %responders vs non-responders
...
for ID_opt = Ind{i_treat}
    ...
    clearvars -except C Ind i_treat ID_check i_ind data ID_opt S0 dN pN...
        ...
        t_detect_sympt  max_VB areaB areaBtopeak areaBafterpeak meanB meanBtopeak meanBafterpeak B_sympt B_hosp B_detect...
        Sex_check
    ...
    max_V(ID_opt) = log10(max(y(:,3)));
    max_VB(ID_opt) = y(find(y(:,3) == max(y(:,3))),4);
    ...
    if i_treat == 1
        ...
        %y_new = -(log(y_short/(1.441*10^14)))/(-0.685); %Ke 2021
        y_new_notreat = log10(y_short_notreat);
    end

    Y_all(:,ID_opt) = log10(y(1:501,3));

    y_new_50(ID_opt) =log10(y(501,3));
    ...
    % end
    VL_all = log10(y(2:501,3));
    VL_topeak = log10(y(2:ind_max_y(ID_opt),3));
    VL_afterpeak = log10(y(ind_max_y(ID_opt):501,3));
    areaVL(ID_opt) = sum((VL_all(VL_all>2.1)-2.1)*0.1);
    areaVLtopeak(ID_opt) = sum((VL_topeak(VL_topeak>2.1)-2.1)*0.1);
    areaVLafterpeak(ID_opt) = sum((VL_afterpeak(VL_afterpeak>2.1)-2.1)*0.1);
    meanVL(ID_opt) = areaVL(ID_opt)*10/sum(VL_all>2.1);
    meanVLtopeak(ID_opt) = areaVLtopeak(ID_opt)*10/sum(VL_topeak>2.1);
    meanVLafterpeak(ID_opt) = areaVLafterpeak(ID_opt)*10/sum(VL_afterpeak>2.1);

    B_all = y(2:501,4);
    B_topeak = y(2:ind_max_y(ID_opt),4);
    B_afterpeak = y(ind_max_y(ID_opt):501,4);
    areaB(ID_opt) = sum(B_all)*0.1;
    areaBtopeak(ID_opt) = sum(B_topeak)*0.1;
    areaBafterpeak(ID_opt) = sum(B_afterpeak)*0.1;
    % areaBafterpeak(ID_opt) = sum(B_afterpeak(VL_topeak>2.6))*0.1;
    meanB(ID_opt) = areaB(ID_opt)*10/length(B_all);
    meanBtopeak(ID_opt) = areaBtopeak(ID_opt)*10/length(B_topeak);
    meanBafterpeak(ID_opt) = areaBafterpeak(ID_opt)*10/length(B_afterpeak);
    % meanBafterpeak(ID_opt) = areaBafterpeak(ID_opt)*10/sum(VL_afterpeak>2.6);
    ...
    CN_sympt(ID_opt) = log10(y(find(tspan==t_shift(ID_opt)),3));
    if ismember(i_treat,[1,9])
        CN_hosp(ID_opt) = log10(y(find(tspan==t_shift(ID_opt)+T_hosp_check(ID_opt)),3));
    end
    if ismember(i_treat,[3])
        if ismember(ID_opt,inds_hosp)
            CN_hosp(ID_opt) = log10(y(find(tspan==t_shift(ID_opt)+T_hosp_check(ID_opt)),3));
        end
    end
    %CN_sympt(ID_opt) = y(find(tspan==t_shift),3);

     B_sympt(ID_opt) = y(find(tspan==t_shift(ID_opt)),4);
     % CN_sympt(ID_opt) = R(find(tspan==t_shift(ID_opt)));
    if ismember(i_treat,[1,9])
        B_hosp(ID_opt) = y(find(tspan==t_shift(ID_opt)+T_hosp_check(ID_opt)),4);
    end

    t_detect(ID_opt) = (find(Y_all(1:find(y(:,3) == max(y(:,3))),ID_opt)<2.1,1,'last')-1)*0.1;
    t_detect_sympt(ID_opt) = (find(Y_all(1:find(y(:,3) == max(y(:,3))),ID_opt)<2.1,1,'last')-1)*0.1-t_shift(ID_opt);

    B_detect(ID_opt) = y(find(Y_all(1:find(y(:,3) == max(y(:,3))),ID_opt)<2.1,1,'last'),4);

    n = sum(~isnan(CT_check{ID_opt}))+sum(~isnan(CT1_check{ID_opt}))+sum(~isnan(CT3_check{ID_opt})); %number of data points

    y_new_comp = (y_new-11.35)/0.25; %Ke 2022
    y_new_notreat_comp = (y_new_notreat-11.35)/0.25; %Ke 2022

    %determine value of negative logL
    % logL = 0.5*n*log(2*pi*sigma^2)+0.5*nansum((data1-y_new).^2./sigma^2);
    MSE(ID_opt) = 1/n*(nansum((-CT_check{ID_opt}-y_new_comp).^2)+nansum((-CT1_check{ID_opt}-y_new_comp).^2)+nansum((-CT3_check{ID_opt}-y_new_comp).^2));
    ME(ID_opt) = 1/n*(nansum(abs(-CT_check{ID_opt}-y_new_comp))+nansum(abs(-CT1_check{ID_opt}-y_new_comp))+nansum(abs(-CT3_check{ID_opt}-y_new_comp)));
    ...
    MSE_data(ID_opt) = 1/(sum(~isnan([(-CT_check{ID_opt}+CT1_check{ID_opt}),(-CT_check{ID_opt}+CT3_check{ID_opt})])))*(nansum(abs(-CT_check{ID_opt}+CT1_check{ID_opt}))+nansum(abs(-CT_check{ID_opt}+CT3_check{ID_opt})));
     ...
     if i_treat == 1
         ...
         logL_wtreat = 0.5*n*log(2*pi*sigma(ID_opt)^2)+0.5*nansum((-CT1_check{ID_opt}-y_new_comp).^2./sigma(ID_opt)^2)+...
             0.5*nansum((-CT_check{ID_opt}-y_new_comp).^2./sigma(ID_opt)^2)+0.5*nansum((-CT3_check{ID_opt}-y_new_comp).^2./sigma(ID_opt)^2);

         BIC_wtreat(ID_opt) = 5*log(n)+2*logL_wtreat;

         logL_notreat = 0.5*n*log(2*pi*sigma_notreat(ID_opt)^2)+0.5*nansum((-CT1_check{ID_opt}-y_new_notreat_comp).^2./sigma_notreat(ID_opt)^2)+...
             0.5*nansum((-CT_check{ID_opt}-y_new_notreat_comp).^2./sigma_notreat(ID_opt)^2)+0.5*nansum((-CT3_check{ID_opt}-y_new_notreat_comp).^2./sigma_notreat(ID_opt)^2);

         BIC_notreat(ID_opt) = 4*log(n)+2*logL_notreat;

         ME_notreat(ID_opt) = 1/n*(nansum(abs(-CT_check{ID_opt}-y_new_notreat_comp))+nansum(abs(-CT1_check{ID_opt}-y_new_notreat_comp))+nansum(abs(-CT3_check{ID_opt}-y_new_notreat_comp)));
...
elseif i_treat == 9
    %inds1 = intersect(inds_treat_hosp,inds_wo_dead);
    inds1 = intersect(inds_treat_hosp,find(alpha>0 & alpha<0.9));
    %inds2 = intersect(inds_treat_hosp,inds_dead);
    inds2 = intersect(inds_treat_hosp,find(alpha>=0.9));
else
    inds1 = intersect(inds_wo_treat,inds_wo_hosp);
    inds2 = union(inds_treat_hosp,intersect(inds_wo_treat,inds_hosp));
end

Sex1 = Sex_check(inds1);
Sex2 = Sex_check(inds2);
...
if ismember(i_treat, [1,9])
    ...
    B_hosp1 = B_hosp(inds1);
end
sigma1 = sigma(inds1);
MSE_data1 = MSE_data(inds1);

peak_B1 = max_VB(inds1);
areaB1 = areaB(inds1);
areaBtopeak1 = areaBtopeak(inds1);
areaBafterpeak1 = areaBafterpeak(inds1);
meanB1 = meanB(inds1);
meanBafterpeak1 = meanBafterpeak(inds1);
meanBtopeak1 = meanBtopeak(inds1);
B_sympt1 = B_sympt(inds1);
B_detect1 = B_detect(inds1);
t_detect_sympt1 = t_detect_sympt(inds1);
...
if ismember(i_treat, [1,9])
    ...
    B_hosp2 = B_hosp(inds2);
end
sigma2 = sigma(inds2);
MSE_data2 = MSE_data(inds2);

peak_B2 = max_VB(inds2);
areaB2 = areaB(inds2);
areaBtopeak2 = areaBtopeak(inds2);
areaBafterpeak2 = areaBafterpeak(inds2);
meanB2 = meanB(inds2);
meanBafterpeak2 = meanBafterpeak(inds2);
meanBtopeak2 = meanBtopeak(inds2);
B_sympt2 = B_sympt(inds2);
B_detect2 = B_detect(inds2);
t_detect_sympt2 = t_detect_sympt(inds2);
...
% end
sprintf('percentage of pop1 with undetectable VL at 50 dpi = %d', V50_1_bin/length(V50_1))
sprintf('percentage of pop2 with undetectable VL at 50 dpi = %d', V50_2_bin/length(V50_2))
...
if i_treat == 1
    inds_par = [{1:4},{5:7},{8:10},{11:13},{14:16},{17:20},{21:23},{24:26},27,28,29];
elseif i_treat == 3
    inds_par = [{1:3},{4:5},{6:7},{8:10},{11:13},{14:16},{17:19},{20:22},23,24];
elseif i_treat == 6
    inds_par = [{1:3},{4:5},{6:8},{9:10},{11:13},{14:16},{17:19},{20:22},23,24];
elseif i_treat == 9
    inds_par = [{1:4},{5:7},{8:10},{11:13},{14:16},{17:20},{21:23},{24:26},27,28,29];
end

sD = 15;
C = [{[95,211,141]./255}, {[95,95,211]./255}, {[255,127,42]./255}, {[204,0,255]./255}, {[85,85,255]./255}, {[95,211,141]./255}, {[0,0,0]}, {[200,200,200]./255}, {[255,179,0]./255}, {[100,100,100]./255}];
M = {'o','^'};
...
figure
t = tiledlayout(4,11,'TileSpacing','Compact','Padding','Compact');
...
for i = 1:length(inds_par)
    ...
    for j = inds_par{i}
        ...
        % else
             ...
             % scatter(P1(j,find(Sex1==0)),-((i_count-1)*2+1)+n1(find(Sex1==0)),'o','MarkerEdgeColor','none','SizeData',sD,'MarkerFaceColor','r','MarkerFaceAlpha',FA)
             % hold on
        % end
        ...
        % scatter(P2(j,find(Sex2==0)),-((i_count-1)*2+1.5)+n2(find(Sex2==0)),'o','MarkerEdgeColor','none','SizeData',sD,'MarkerFaceColor','r','MarkerFaceAlpha',FA)
        %      hold on
        ...
        % else
            ...
            sprintf('2.5th percentile of par_%d: pop1 %d vs pop2 %d', j, prctile(P1(j,:),2.5), prctile(P2(j,:),2.5))
            sprintf('97.5th percentile of par_%d: pop1 %d vs pop2 %d', j, prctile(P1(j,:),97.5), prctile(P2(j,:),97.5))
            K_all_r(j) = round(p,2);
            M_all_r(j) = round(mediantest(P1(j,:),P2(j,:)),2);
            K_all(j) = p;
            M_all(j) = mediantest(P1(j,:),P2(j,:));
            M_1(j) = median(P1(j,:));
            M_2(j) = median(P2(j,:));
            P25_1(j) = prctile(P1(j,:),2.5);
            P975_1(j) = prctile(P1(j,:),97.5);
            P25_2(j) = prctile(P2(j,:),2.5);
            P975_2(j) = prctile(P2(j,:),97.5);
        % end

    end

    ylim([-8,0])
    if ismember(i,[1,3,4,5,7,9])
        xlim([0,Inf])
    end
...
% elseif i_treat == 9
%     hgexport(gcf,'Par_analysis_response.pdf',figure_property); %Set desired file name
% else
...
%         hold on
%         gscatter(Y(size(P1,2)+(1:size(P2,2)),1),Y(size(P1,2)+(1:size(P2,2)),2),species(size(P1,2)+(1:size(P2,2))),[C{1};C{2}],'.',10,'off')
%         hold on
%         % alpha2 = alpha(inds2);

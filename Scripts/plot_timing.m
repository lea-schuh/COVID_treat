...
for i_ind = 1:length(inds_wo_treat_wo_hosp)
    ...
    hold on
    plot(T_check{inds_wo_treat_wo_hosp(i_ind)} + OnsetSympt_check(inds_wo_treat_wo_hosp(i_ind)),-i_count,'.','Color', C{1});
    hold on    
    i_count = i_count+1;
...
for i_ind = 1:length(inds_wo_treat_hosp)
    ...
    yline(-i_count,'Color', C{5});
    hold on
    plot(T_check{inds_wo_treat_hosp(i_ind)} + OnsetSympt_check(inds_wo_treat_hosp(i_ind)),-i_count,'.','Color', C{2});
    hold on
    i_count = i_count+1;
...
for i_ind = 1:length(inds_treat_hosp_wo_dead)
    ...
    yline(-i_count,'Color', C{5});
    hold on
    plot(T_check{inds_treat_hosp_wo_dead(i_ind)} + OnsetSympt_check(inds_treat_hosp_wo_dead(i_ind)),-i_count,'.','Color', C{4});
    hold on
    i_count = i_count+1;
...
for i_ind = 1:length(inds_treat_hosp_dead)
    ...
    yline(-i_count,'Color', C{5});
    hold on
    plot(T_check{inds_treat_hosp_dead(i_ind)} + OnsetSympt_check(inds_treat_hosp_dead(i_ind)),-i_count,'.','Color', C{3});
    hold on
    i_count = i_count+1;
...
set(h1, 'Position',  [100, 100, 200, 450])
